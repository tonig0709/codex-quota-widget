...
download against the checksums file attached to each GitHub Release. A DMG
improves installation but does not change this signing status. A local process
could still impersonate the fixed loopback port; the impact is limited to
displaying false quota data, not account access or token exposure.
