...
- **Widget discovery:** the gate seeds an enabled lower-version/build registration,
  ...
  requires the App to remove the obsolete containing App by its concrete
  LaunchServices path and elect exactly one enabled current extension, without
  using `pluginkit -r` on the shared widget identifier. AppIntent metadata, both
